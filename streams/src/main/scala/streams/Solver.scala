...
trait Solver extends GameDef {
  ...
  /**
    ...
    */
    def from(initial: Stream[(Block, List[Move])],
             explored: Set[Block]): Stream[(Block, List[Move])] = {
      if (initial.isEmpty) return Stream()
      val paths = initial.flatMap {
        case (block, hist) => newNeighborsOnly(neighborsWithHistory(block, hist), explored)
      }

      paths ++: from(paths, explored ++ paths.map(_._1))
    }
  ...
  lazy val pathsFromStart: Stream[(Block, List[Move])] =
    from(neighborsWithHistory(startBlock, List()), Set())
